...
def producedata(n=50, f_path = "data/digitdata/trainingimages",l_path="data/digitdata/traininglabels",dimension = 28):
    import doctest
    doctest.testmod()  # Test the interactive sessions in function comments

    #  items = loadDataFile("facedata/facedatatrain", n,60,70)
    #  labels = loadLabelsFile("facedata/facedatatrainlabels", n)
    items = loadDataFile(f_path, n,0 ,dimension)            # n images and it's diminsions
    labels = loadLabelsFile(l_path, n)                 #labels
    features = []

    for i in range(n):


        features.append( items[i].pixels)


    features = np.array(features)
    features = features.reshape(features.shape[0], features.shape[2] * features.shape[1])  # convert 3d to 2d array
...
if __name__ == "__main__":
    ...
    #a,b=producedata(dimension=28)
    features, targets = producedata(10,"data/facedata/facedatatrain","data/facedata/facedatatrainlabels",60)
